package com.USALynch.machineLearning.neuralNetwork.networks.breeze
...
import com.USALynch.machineLearning.neuralNetwork.networks.BreezeFeedForwardNetwork
 ...
 */
class MultiOutput_Breeze_FeedForwardNetworkSpec extends FlatSpec with Matchers {
   ...
   */
  val neuralNetwork = new BreezeFeedForwardNetwork(network)
  val learningRate = 0.01d
  val learningRate2 = 0.750d
  ...
  it should "be able to forward propagate correctly" in {
    (clazz_1.map(_.result).getOrElse(Seq.empty[Double]) zip Array(0.879,0.816)).map{ case (in, out) =>
  ...
  it should "be able to store and compute valid activation values" in {
    (lastLayer_1.nodes.head.inputs.get zip Array(0.990,0.990)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }
  ...
  it should "be able to update the weights in the first node in the last layer" in {
    (lastLayer.nodes.head.inputWeights.map(_.weight) zip Array(0.700,0.800)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }
  it should "be able to update the weights in the last node in the last layer" in {
    (lastLayer.nodes.last.inputWeights.map(_.weight) zip Array(0.898,0.098)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }
  ...
  it should "be able to update the weights in the first node in the hidden layer" in {
    (hiddenLayer.nodes.head.inputWeights.map(_.weight) zip Array(0.1,0.500,0.400)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }
  it should "be able to update the weights in the last node in the hidden layer" in {
    (hiddenLayer.nodes.last.inputWeights.map(_.weight) zip Array(0.300,0.200,0.6)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }
  ...
  it should "be to reclassify with the weights and get a new value that is known to be valid out." in {
    (clazz.map(_.result).getOrElse(Seq.empty[Double]) zip Array(0.887,0.797)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }

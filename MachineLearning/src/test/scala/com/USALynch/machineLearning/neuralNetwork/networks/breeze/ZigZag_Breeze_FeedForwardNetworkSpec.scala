 ...
 */
class ZigZag_Breeze_FeedForwardNetworkSpec extends FlatSpec with Matchers {
  ...
  it should "be able to forward propagate correctly" in {
    (clazz_1.map(_.result).getOrElse(Seq.empty[Double]) zip Array(0.803)).map{ case (in, out) =>
  ...
  it should "be to reclassify with the weights and get a new value that is known to be valid out." in {
    (clazz.map(_.result).getOrElse(Seq.empty[Double]) zip Array(0.800)).map{ case (in, out) =>
      in should be (out +- eps)
    }
  }

...
/**
  ...
  *
  * The Linear Algebra Library used for this implementation is contained within scala.breeze
  *     https://github.com/scalanlp/breeze/wiki/Linear-Algebra-Cheat-Sheet
  *
  * @author Tyler T. Lynch
  */
class BreezeFeedForwardNetwork(
  networkSeed: Network,
  ...
  private type RealMatrix = DenseMatrix[Double]
  ...
  def feedForward(input: Seq[Double]): Future[Classification] = {
    for {
      ...
      assert(input.length == firstLayerBias.rows)
      ...
      val networkWithWeights: Seq[FeedForwardState] = network.layers.drop(1).foldLeft(Seq(seed)) { case (previousLayers, layer) =>
        ...
        val sub_weightedInput: RealMatrix = rawWeights * in
  ...
  private def toSingleColumnMatrix(input: Iterable[Double]): RealMatrix = {
    toColumnMatrix(Array(input))
  }


  private def toColumnMatrix(input: Iterable[Iterable[Double]]): RealMatrix = {
    val rows = input.size
    val cols = input.headOption.map(_.size).getOrElse(throw new Exception("You need data to create a matrix."))
    new DenseMatrix(rows, cols, input.flatten.toArray).t
  }

  private def toWeightMatrix(nodes: Seq[NetworkNode]): RealMatrix = {
    val rows = nodes.length
    val cols = nodes.headOption.map(_.inputWeights.length).getOrElse(throw new Exception("You need data to create a matrix."))
    val columnMajor: Array[Double] = nodes.flatMap( node => node.inputWeights.map(_.weight) ).toArray
    new DenseMatrix(rows, cols, columnMajor)
  }
  ...
  def computeLayerAdjustments(
    ...
    val gradient_ws: RealMatrix = dws * gradient_bs.t

    val changes_bs: RealMatrix = gradient_bs :*= learningRate
    val changes_ws: RealMatrix = (gradient_ws :*= learningRate).t
  ...
  private def hadamardProduct(a: RealMatrix, b: RealMatrix): RealMatrix = {
    val at = a.t
    val bt = b.t
    val aRows: Map[Int, Array[Double]] = (0 until at.cols).map(i => i -> at(::, i).toArray).toMap
    val rows: Iterable[Iterable[Double]] = (0 until bt.cols).map(i => i -> bt(::, i).toArray).map{ case (index, row) =>
      (row zip aRows(index)).map{ case (one, two) => one*two}.toIterable
    }
    toColumnMatrix(rows).t
  }
  ...
  def adjustLayer(
     ...
     */
    val weights: RealMatrix = toColumnMatrix( layerToAdjust.nodes.map(x => {
      x.inputWeights.map(_.weight)
    }) )
    val sub_newWeights: RealMatrix = weights - weightChanges.t
    val newWeights: RealMatrix = sub_newWeights
    val weightMap: Map[Int, Seq[InputWeight]] = (0 until newWeights.cols).map{ index =>
      val layer = newWeights(::, index).toArray
